...
function formatLevel(key) {
  if (!key) return "No level assigned";
  return LEVEL_LABELS[String(key).toLowerCase()] || key;
}
...
function flightOptions(activities, selectedFlightId = "") { return activities.map(activity => `<optgroup label="${escapeHtml(activity.name)}">${(activity.flights || []).map(flight => `<option value="${escapeHtml(flight.id)}" ${flight.id === selectedFlightId ? "selected" : ""}>${escapeHtml(flight.name)} (${escapeHtml(formatLevel(flight.name))})</option>`).join("")}</optgroup>`).join(""); }
...
export async function superAdminSessionLogView() {
  requireSuperAdmin();
  try {
    const sessions = await api("/sessions/all").catch(() => api("/timetable/mine"));
      ...
      <section class="card table-wrap">
        <table class="schedule">
          ...
          <tbody>
            ${allSessions.map(s => `
              <tr>
                <td><b>${escapeHtml(recordDate(s.startAt))}</b></td>
                <td><span class="tag blue">${escapeHtml(formatLevel(s.flightName || s.activityName))}</span></td>
                ...
                <td><span class="tag ${s.status === 'COMPLETED' ? 'blue' : 'amber'}">${escapeHtml(s.status || 'SCHEDULED')}</span></td>
              </tr>
            `).join('') || "<tr><td colspan='6'>No session records found across the club.</td></tr>"}
          </tbody>
        </table>
      </section>`;
  } catch (err) {
    return superAdminAuditLogView({ title: "Whole Club Attendance Sessions", description: "All attendance and completed-game records from every flight and level.", categories: ["ATTENDANCE", "SESSION CONTROL"] });
  }
}
...
export async function financeAdminView() {
  ...
  const creditsContent = `
    <section class="card">
      ...
      <p class="note">First select Activity and Flight / Level above. The Member list below shows only active members assigned to that selection.</p>
      <div class="field">
        <label for="financeCreditMember">Member</label>
        <select id="financeCreditMember" required>
          <option value="" disabled ${selectedFinanceCreditMemberId ? "" : "selected"}>Choose member${selectedFinanceFlightId ? " in selected level" : selectedFinanceActivityId ? " in selected activity" : ""}</option>
          ${creditRecipients.map(member => `<option value="${escapeHtml(member.uid)}" data-wallet-balance="${Number((data.credits || []).find(row => row.memberUid === member.uid)?.balanceFils || 0)}" ${member.uid === selectedFinanceCreditMemberId ? "selected" : ""}>${escapeHtml(member.fullName)} · ${escapeHtml(member.memberId || "No ID")} · ${escapeHtml(formatLevel(member.flightName || flights.find(flight => flight.id === member.flightId)?.name))} · ${escapeHtml(String(member.role || "PLAYER").replaceAll("_", " "))}</option>`).join("")}
        </select>
        <p id="financeSelectedMemberPreview" class="note">${selectedFinanceCreditMemberId ? `Selected member: <b>${escapeHtml(creditRecipients.find(member => member.uid === selectedFinanceCreditMemberId)?.fullName || "selected member")}</b><br>Current wallet credit: <b id="financeCurrentCredit">${bhd(selectedCreditBalanceFils)}</b>` : creditRecipients.length ? "Choose the exact member before adding or deducting credit." : "No active member is assigned to this activity / level yet."}</p>
      </div>
    </section>
    ...
    <div class="grid two">
      ...
      <section class="card">
        ...
        <button id="deductFinanceCredit" class="primary" style="background:#dc2626; border-color:#991b1b;" ${selectedFinanceCreditMemberId ? "" : "disabled"}>➖ Deduct wallet credit</button>
      </section>
    </div>
...
export function bindAdminViews() {
  const adSections = [...document.querySelectorAll("[data-ad-section]")];
  const adPanels = [...document.querySelectorAll("[data-ad-panel]")];
  const showAdPanel = panelId => {
    adSections.forEach(section => section.hidden = section.id !== panelId);
    adPanels.forEach(button => button.classList.toggle("primary", button.dataset.adPanel === panelId));
  };
  if (adSections.length) {
    const initialPanel = adPanels[0]?.dataset.adPanel || adSections[0].id;
    showAdPanel(initialPanel);
    adPanels.forEach(button => button.onclick = () => showAdPanel(button.dataset.adPanel));
  }
  ...
  document.querySelectorAll("[data-print-finance], [data-print-audit], [data-print-advertising], [data-print-deleted-users]").forEach(button => button.onclick = () => window.print());
  ...
  document.querySelectorAll("[data-delete-ad]").forEach(button => button.onclick = async () => {
    try {
      ...
      notify("Advertisement record deleted.");
      refresh();
    } catch (error) { notify(error.message); }
  });
