...
function flightOptions(activities, selectedFlightId = "") { return activities.map(activity => `<optgroup label="${escapeHtml(activity.name)}">${(activity.flights || []).map(flight => `<option value="${escapeHtml(flight.id)}" ${flight.id === selectedFlightId ? "selected" : ""}>${escapeHtml(formatLevelName(flight.name))}</option>`).join("")}</optgroup>`).join(""); }
...
export async function superAdminSessionLogView() {
  ...
  return `<div class="page-head"><div><span class="tag blue">SUPER ADMIN</span><h2>Whole Club Attendance Sessions</h2><p>View whole club attendance, session completions, and shuttle usage across every level.</p></div></div>
    <section class="card">
      <div class="field">
        <label for="sessionControlFlightFilter">Level Filter Dropdown</label>
...
export async function financeAdminView() {
  ...
  const creditsContent = `
    <section class="card">
      ...
      <p class="note">First select Activity and Flight / Level above. The Member list below then shows only active members assigned to that selection.</p>
      <div class="field">
