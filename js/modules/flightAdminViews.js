...
export async function flightAdminSessionControlView() {
  ...
  const sessionCards = today.map(session => {
    ...
    const adminSelfMarkup = adminSelfRow ? `
      <div class="card" style="background:#eff6ff; border:1px solid #bfdbfe; margin-bottom:12px; padding:12px;">
        <div class="page-head" style="margin:0;">
          ...
          <div class="actions">
            <button class="pill ${adminSelfRow.status === 'PRESENT' ? 'primary' : ''}" data-self-attendance="PRESENT" data-session-id="${escapeHtml(session.id)}">I'm Playing (Present)</button>
            <button class="pill ${adminSelfRow.status === 'ABSENT' ? 'danger-action' : ''}" data-self-attendance="ABSENT" data-session-id="${escapeHtml(session.id)}">Not Playing (Absent)</button>
          </div>
        </div>
      </div>` : "";
...
export function bindFlightAdminViews() {
  ...
  document.querySelectorAll("[data-complete-flight-game]").forEach(button => button.onclick = async () => {
    try {
      ...
      if (!Number.isInteger(actualShuttlesUsed) || actualShuttlesUsed < 0) throw new Error("Enter a whole number of shuttlecocks used.");
      const result = await api(`/finance/session/${encodeURIComponent(sessionId)}/complete`, { method: "POST", body: { actualShuttlesUsed, flightId } });
