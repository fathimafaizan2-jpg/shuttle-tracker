...
export async function flightAdminSessionControlView() {
  ...
  const sessionCards = today.map(session => {
    ...
    return `<article class="session"><div class="grow"><b>${escapeHtml(session.flightName || state.member.flightName || "Assigned flight")}</b><p>${escapeHtml(dateTime(session.startAt))} – ${escapeHtml(dateTime(session.endAt))}</p></div><span class="tag ${completed ? "blue" : ended ? "amber" : "blue"}">${completed ? "COMPLETED" : ended ? "READY TO FINISH" : "TODAY · UPCOMING"}</span>${ended && !completed ? `<div class="grid two"><div class="field"><label for="actualShuttlesUsed-${escapeHtml(session.id)}">Shuttles used</label><input id="actualShuttlesUsed-${escapeHtml(session.id)}" type="number" min="0" step="1" placeholder="Number used" /></div><button class="primary" data-complete-flight-game="${escapeHtml(session.id)}">Finish game and create Player charges</button></div>` : ""}</article>`;
