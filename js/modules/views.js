...
export async function playerTimetable() {
  ...
  const rows = dayNames.map((dayName, dayIndex) => {
    const daySlots = safeSlots.filter(slot => Number(slot.weekdayIndex) === dayIndex);
...
export async function playerActivityLog() {
  const log = await api("/members/activity-log").catch(() => ({ gameDays: [], attendance: [], charges: [], payments: [], credentialUpdates: [] }));
  ...
  return `<div class="page-head"><div><h2>User Activity Logs</h2><p>Choose the information you need.</p></div></div><section class="card"><div class="actions">${Object.entries(sections).map(([key, section]) => `<button class="${key === activityLogSection ? "primary" : "pill"}" data-log-section="${key}">${section.label}</button>`).join("")}</div></section><section class="card"><h3>${selected.label}</h3>${selected.content}</section>`;
}

export async function walletView() {
  const data = await api("/finance/mine").catch(() => ({ balanceFils: 0, unpaidFils: 0, arrearsFils: 0, payments: [], charges: [], ledger: [] }));
...
export async function publicIndiMart() {
  const businesses = await api("/business/public/directory").catch(() => []);
  ...
  const card = business => {
    ...
    const imageMarkup = image ? (destination
      ...
      : `<div class="directory-flyer directory-flyer-empty"><span>BAZAAR</span></div>`;
  ...
  if (submitBusiness) submitBusiness.onclick = async () => {
    try {
      const result = await api("/business/public/submit", { method: "POST", body: {
        ...
        email: document.getElementById("memberBizEmail").value.trim(),
